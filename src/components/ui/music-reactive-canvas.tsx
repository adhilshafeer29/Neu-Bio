...
export const MusicReactiveCanvas: React.FC = () => {
  ...
  const initCanvas = useCallback(() => {
    ...
    const beam = {
      ...
      waves: [
        { 
          amplitude: 30, 
          frequency: 0.003, 
          speed: 0.005, // Decreased from 0.02
          offset: 0,
          thickness: 1,
          opacity: 0.9
        },
        { 
          amplitude: 25, 
          frequency: 0.004, 
          speed: 0.004, // Decreased from 0.015
          offset: Math.PI * 0.5,
          thickness: 0.8,
          opacity: 0.7
        },
        { 
          amplitude: 20, 
          frequency: 0.005, 
          speed: 0.006, // Decreased from 0.025
          offset: Math.PI,
          thickness: 0.6,
          opacity: 0.5
        },
        { 
          amplitude: 35, 
          frequency: 0.002, 
          speed: 0.003, // Decreased from 0.01
          offset: Math.PI * 1.5,
          thickness: 1.2,
          opacity: 0.6
        }
      ...
      particles: [] as any[],
      bassHistory: new Array(20).fill(0) as number[],
      postProcessing: {
    ...
    const animate = () => {
      ...
      // Clear canvas - fully opaque to prevent blinking
      ...
      if (analyserRef.current && isPlaying) {
        ...
        // Demo animation - slower
        beam.bassIntensity = 0.4 + Math.sin(beam.time * 0.005) * 0.3 // Slower
        beam.midIntensity = 0.3 + Math.sin(beam.time * 0.008) * 0.2 // Slower
        beam.trebleIntensity = 0.2 + Math.sin(beam.time * 0.01) * 0.1 // Slower

        beam.colorState.targetHue = 180 + Math.sin(beam.time * 0.003) * 180 // Slower
        beam.colorState.targetSaturation = 70 + Math.sin(beam.time * 0.005) * 30 // Slower
        beam.colorState.targetLightness = 50 + Math.sin(beam.time * 0.004) * 20 // Slower
      }
      ...
      // 6. Subtle color grading (flicker effect removed to prevent blinking)
      ctx.save()
      ctx.globalCompositeOperation = 'overlay'
      ctx.globalAlpha = 0.1
